...
// 단일 컨테이너 삭제
export const dockerRmSchema: DockerToolSchema = {
  name: "docker_rm",
...
export async function dockerRm(input: { container: string }): Promise<{ content: { type: string; text: string }[] }> {
...
// 다중 컨테이너 삭제
export const dockerRmMultiSchema: DockerToolSchema = {
...
export async function dockerRmMulti(input: { containers: string[] }): Promise<{ content: { type: string; text: string }[] }> {
